...
max_steer_angle = 4;  % [deg] max steering angle at the front wheel % changable 
min_turn_radius = L/tan(deg2rad(max_steer_angle));

% ---------------------- 
% Solve the problem
% ----------------------
planner = pathPlannerRRT(costmap,'ConnectionDistance',40,...
        'GoalBias',0.1,'MinIterations',1e3,'MaxIterations',1e4,...
...
% interpolate the calculated reference path --- opt 
refRoute_points_orig = interpolate(refPath);
interp_sampling = 10; % changable -- sampling distance 
